...
#define SLAVE_NO_CONNECTED       0
#define SLAVE_WAITING_CONNECTED  1
...
static uint8_t slavenotifieddata[100];  //roger only for temp test

static uint8_t latestconnectedindex;
...
static uint8_t slaves_local_name[] = {AD_TYPE_COMPLETE_LOCAL_NAME,'6','L','P','_','B','L','E','1'}; 

static uint8_t slaves_local_name2[] = {AD_TYPE_COMPLETE_LOCAL_NAME,'L','Y','W','S','D','0','3','M','M','C'}; 
...
void set_database(void)
{
  ...
  status = aci_gatt_add_char(service_handle,UUID_TYPE_16,&char_uuid,CHAR_LEN,CHAR_PROP_READ|CHAR_PROP_NOTIFY|CHAR_PROP_WRITE|CHAR_PROP_WRITE_WITHOUT_RESP,ATTR_PERMISSION_NONE,GATT_NOTIFY_ATTRIBUTE_WRITE,0x07,0x01, &char_handle);
  if (status != BLE_STATUS_SUCCESS) {
...
uint8_t set_connection(void)
{
  ...
  for(connectioncounter =0; connectioncounter < discovery_counter;connectioncounter++)
  {
      if(discovery[connectioncounter].connectionstate == SLAVE_WAITING_CONNECTED)
        break;
  }
  ...
  status = aci_gap_create_connection(MS_Connection_Parameters.Scan_Interval,
                                     MS_Connection_Parameters.Scan_Window,
                                     discovery[connectioncounter].device_found_address_type,
                                     discovery[connectioncounter].device_found_address,
                                     0x00,
...
void write_characteristic_descriptor(uint16_t Connection_Handle)
{
  ...
  if(discovery[slaveindex].device_found_address[0] == 0x7f)
  {
     atrribute_handle = 0x0038;
  }
  else
    atrribute_handle = 0x000f;
     ...
     while(char_len > 0)
      {
         ...
         if(char_len >= update_len)
          { 
             status=aci_gatt_update_char_value_ext(master_connection_handle[i], service_handle,char_handle,1,update_len,0x00,update_len,&(master_char_value[dataoffset])); 
             if (status != BLE_STATUS_SUCCESS)
             {
                PRINTF("aci_gatt_update_char_value_ext FAILED ");
                return;
             }
             char_len = char_len - update_len;      
             dataoffset = dataoffset + update_len;
          }
          else
          {
              status=aci_gatt_update_char_value_ext(master_connection_handle[i], service_handle,char_handle,1,update_len,0x00,update_len,&(master_char_value[dataoffset])); 
...
void APP_Tick(void)
{
  ...
  if (APP_FLAG(DO_SLAVES_CONNECTION)) 
  {
     ...
     else if APP_FLAG(NOTIFICATIONS_ENABLED)
    {
      ...
      write_characteristic_descriptor(discovery[latestconnectedindex].connection_handle);
    }
  ...
  while (index < data_length) {
    ...
    if (data_value[index+1] == AD_TYPE_COMPLETE_LOCAL_NAME) { 
      ...
      if (memcmp(&data_value[index+1], &slaves_local_name[0], SLAVES_LOCAL_NAME_LEN) == 0)
        return TRUE;
      else if(memcmp(&data_value[index+1], &slaves_local_name2[0], SLAVES_LOCAL_NAME_LEN) == 0)
         return TRUE;
      else
        return FALSE;
    } else {
...
static uint8_t Address_DiscoveryIndex_Check(uint8_t *bdaddr)
{
  uint8_t i; 
...
void hci_le_advertising_report_event(uint8_t num_reports,Advertising_Report_t advertising_report[])
{
 ...
 // uint8_t evt_type = advertising_report[0].Event_Type ;
  ...
  if((bdaddr[5] == 0x03)&&(bdaddr[4] == 0x80))
  {
       PRINTF("advertising address: %x %x %x %x %x %x\r\n",bdaddr[5],bdaddr[4],bdaddr[3],bdaddr[2],bdaddr[1],bdaddr[0]);
      PRINTF("advertising name: %x %x %x %x %x %x\r\n",advertising_report[0].Data[0],advertising_report[0].Data[1],advertising_report[0].Data[2],advertising_report[0].Data[3],advertising_report[0].Data[4],advertising_report[0].Data[5]); 
  }
  
  /*  check current found device */
//roger   if ((evt_type == ADV_IND) && Is_New_Device(bdaddr) && Find_DeviceName(data_length, advertising_report[0].Data) )
  if(Find_DeviceName(data_length, advertising_report[0].Data))
  {
    if(Is_New_Device(bdaddr)) 
    {
       /* store first device found:  address type and address value */
        ...
        discovery[discovery_counter].connectionstate = SLAVE_WAITING_CONNECTED;
        discovery_counter +=1;
        actual_discovered_slave_devices +=1; 
    }
    else   //the old address was found again
    {
        discoveryindex = Address_DiscoveryIndex_Check(bdaddr);
        discovery[discoveryindex].device_found_address_type = bdaddr_type;
        if(discovery[discoveryindex].connectionstate == SLAVE_NO_CONNECTED)
        {
            discovery[discoveryindex].connectionstate = SLAVE_WAITING_CONNECTED;
            actual_discovered_slave_devices +=1; 
        }
    }
  }
...
void hci_le_connection_complete_event(uint8_t status,uint16_t connection_handle,uint8_t role,uint8_t peer_address_type,uint8_t peer_address[6],uint16_t conn_interval,uint16_t conn_latency,uint16_t supervision_timeout,uint8_t master_clock_accuracy)
{
  ...
  if (status == 0x00)
  {
    if (role == 0x00) //(APP_FLAG(WAIT_SLAVE_CONNECTION)) /* CONNECTED as Master with Slaves devices */
    {
      ...
      for (uint8_t i=0; i < discovery_counter; i++)
      {
         if (memcmp(peer_address, &discovery[i].device_found_address, 6) == 0)
         {
             discovery[i].connection_handle  = connection_handle; 
             discovery[i].connectionstate = SLAVE_CONNECTED;  
    ...
    else if (role == 0x01)//  (APP_FLAG(SET_CONNECTABLE))/* CONNECTED as Slave with Master */ 
    {
...
{	
   uint16_t latest_attribute_handle;
   static uint8_t startflag = 0;
  /* Receive Notifications from Slaves and send them to the Master */

  /* value = 0x<slave_index><slave_value> */
   latest_attribute_handle = attribute_handle;
   
   PRINTF("\nGet notification _event attribute_handle:%d  Value_length:%d\r\n",latest_attribute_handle,attribute_value_length);
   
  for(uint8_t i=0;i <= 5;i++)
  {
      if(discovery[i].connection_handle == connection_handle)
      {
          slave_index = i;
          break;
      }
  ...
  PRINTF("\nGet notification _event attribute_handle:%d  Value_length:%d slave index:%d\r\n",latest_attribute_handle,attribute_value_length,slave_index); 
  
  if((attribute_value[2] == 0x32)&&(attribute_value[4] == 0x2c)) //it's meant the first notification of data
  {
      slavenotifieddata[0] = attribute_value[4];  //target received data
      slavenotifieddata[1] = attribute_value_length; //has recevied data  
      Osal_MemCpy(&(slavenotifieddata[2]), attribute_value, attribute_value_length);
      startflag = 0xaa;
  }
  else if(startflag == 0xaa)
  {    
      Osal_MemCpy(&(slavenotifieddata[slavenotifieddata[1]+2]), attribute_value, attribute_value_length);
      slavenotifieddata[1] += attribute_value_length; //has recevied data 
  }
  ...
  if (num_masters>0)
  {
     if(startflag != 0xaa)
     {
         slave_notification_length = attribute_value_length;// slavenotifieddata[0];
         Osal_MemCpy(&(master_char_value[0]), attribute_value,slave_notification_length);
         APP_FLAG_SET(GET_NOTIFICATION); 
     }
     else
     {
         if(slavenotifieddata[1] >= slavenotifieddata[0])
         {
          slave_notification_length =  slavenotifieddata[0];
          Osal_MemCpy(&(master_char_value[0]), &(slavenotifieddata[2]),slavenotifieddata[0]);
          APP_FLAG_SET(GET_NOTIFICATION); 
          startflag = 0;
         }
     }
...
{
  uint8_t  status;
  uint16_t attr_handle_slave;
  PRINTF("aci_gatt_attribute_modified_event --> EVENT\r\n");
  
  attr_handle_slave = 0x0e;
  
  if(actual_discovered_slave_devices >= 1)
  {
      for(uint8_t i=0;i< actual_discovered_slave_devices; i++)
      {
          if((discovery[i].device_found_address[0] == 0x7f)||(discovery[i].device_found_address[0] == 0xe0))
          {
              attr_handle_slave = 0x33;
          }
          else
          {
              attr_handle_slave = 0x0e;
          }
          status = aci_gatt_write_char_value(discovery[i].connection_handle,attr_handle_slave,attr_data_length,attr_data);
          if(status == 0)
          {
              PRINTF("aci_gatt_write_char_value to slave handle:%d,slavenumber: %d\r\n",discovery[i].connection_handle,attr_handle_slave);
          }
          else
          {          
           PRINTF("aci_gatt_write_char_value Fail to slave handle:%d,slavenumber:%d,%d\r\n",discovery[i].connection_handle,attr_handle_slave,status); 
          }
      }
  }
}
...
{
  //PRINTF("hci_disconnection_complete_event --> EVENT conn_handle: %d \r\n",connection_handle);
  
    if(master_connection_handle[0] == connection_handle)
    {
      if(connection_masters_counter >= 1)
      {
        connection_masters_counter = connection_masters_counter - 1;
      }
    ...
    else if(connection_slaves_counter >= 1)
    {
      for(uint8_t i = 0;i < discovery_counter;i++)
      {
          if(discovery[i].connection_handle  == connection_handle)
          {
             discovery[i].connectionstate = SLAVE_NO_CONNECTED;
             connection_slaves_counter = connection_slaves_counter -1;  
             actual_discovered_slave_devices = actual_discovered_slave_devices -1;
          }
      }
    }
}
